...
# data (path)
root = '/Users/gamzekecibas/Desktop/GIT/comp511-project/cyclegan-tutorial/afhq'
...
#criterion_cycle.device
#criterion_identity.device
...
class ImageDataset(Dataset):
    def __init__(self, root, transforms_=None, unaligned=False, mode='train'):
        ...
        self.mode = mode
        if self.mode == 'train':
            self.files_A = sorted(glob.glob(os.path.join(root+'/train/cat')+'/*.*'))
            self.files_B = sorted(glob.glob(os.path.join(root+'/train/dog')+'/*.*'))
        elif self.mode == 'test':
            self.files_A = sorted(glob.glob(os.path.join(root+'/val/cat')+'/*.*'))
            self.files_B = sorted(glob.glob(os.path.join(root+'/val/dog')+'/*.*'))
...
def main():
    wandb.init(project="CGANT_TRIAL", entity="comp511")
    ...
    for epoch in range(epoch, n_epochs):
        for i, batch in enumerate(tqdm(dataloader)):
            ...
            # Set model input
            real_A = batch['A'].type(torch.Tensor)
            real_B = batch['B'].type(torch.Tensor)
            
            # Adversarial ground truths
            valid = torch.Tensor(np.ones((real_A.size(0), *D_A.output_shape))) # requires_grad = False. Default.
            fake = torch.Tensor(np.zeros((real_A.size(0), *D_A.output_shape))) # requires_grad = False. Default.
    ...
    # -----------------
            if (i+1) % 50 == 0:
                wandb.log({"loss_G":loss_G.item(), "loss_D":loss_D.item()})
                wandb.log({"epoch":epoch, "batch":i+1})
                wandb.log({"real_iamge:": wandb.Image(real_A), "fake_image":wandb.Image(fake_B)})
